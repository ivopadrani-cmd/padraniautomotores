...
export default function CompleteReceiptDataForm({ open, onOpenChange, reservation, vehicle, client, onReceiptComplete }) {
  ...
  const getMissingFields = () => {
    ...
    // Verificar vehículo
    if (!vehicle?.kilometers) missing.vehicle.push({ field: 'kilometers', label: 'Kilometraje' });
    if (!vehicle?.color) missing.vehicle.push({ field: 'color', label: 'Color' });
  ...
  const handleSubmit = async (e) => {
    ...
    missingFields.vehicle.forEach(({ field, label }) => {
      const value = vehicleData[field]?.toString().trim();
      if (!value) {
        toast.error(`El campo "${label}" es requerido`);
        hasErrors = true;
      } else {
        vehicleUpdates[field] = field === 'kilometers' ? parseFloat(value) : value;
      }
    });

    // Validar cliente
    missingFields.client.forEach(({ field, label }) => {
      const value = clientData[field]?.toString().trim();
      if (!value) {
        toast.error(`El campo "${label}" es requerido`);
        hasErrors = true;
      } else {
    ...
    try {
      ...
      toast.success('¡Datos completados! Ahora puedes crear el recibo de seña.');
      onOpenChange(false);
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Todos los datos necesarios para el recibo de seña están completos. Puedes crear el recibo ahora.
            </DialogDescription>
          </DialogHeader>
          <div className="flex justify-end gap-2">
            <Button onClick={() => onOpenChange(false)}>Cerrar</Button>
            {onReceiptComplete && (
              <Button onClick={onReceiptComplete} className="bg-cyan-600 hover:bg-cyan-700">
                Ver Recibo
              </Button>
            )}
          </div>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Completar Datos para Recibo de Seña
          </DialogTitle>
          <DialogDescription>
            Para crear el recibo de seña, necesitas completar los siguientes campos faltantes:
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Alert>
            ...
            <AlertDescription>
              Completa todos los campos marcados como requeridos para poder generar el recibo de seña.
            </AlertDescription>
          </Alert>
